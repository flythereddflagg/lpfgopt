...
array_2d* zeros(int rows, int columns)
{
...
/* Run one iteration of the Leapfrog algorithm given the matrix
   ...
   and worst objective function values */
    ...
    matrix->array[wi][0] = func(args); // Update objective
}
...
void minimize(double (*f1_ptr)(double*), array_2d* int_1,
              int npts, int nvars)
{
/* Minimize function that runs the leap_frog algorithm to
   minimize f1_ptr on the intervals on int_1 */
    ...
    for (dvi = 1; dvi < nvars; dvi++){
        printf("%.9f ", pt_set->array[bi][dvi]);
        }
    printf("\n");
