...
def cminimize(f,ivls):
    
    lpfgso = cdll.LoadLibrary("./liblpfg.so")
    ...
    lpfgso.set.argtypes = [FUNC]
    lpfgso.set.restypes = None
    lpfgso.set(f1)

    pfunc = FUNC.in_dll(lpfgso, 'pfunc')

    sol_address = lpfgso.minimize(pfunc, ivlp, 20, len(ivls))
...
def f_test(x): return 2.0 * x[0]**2 + x[1]**2 + 3.0
